...
class BaseModule(nn.Module):
    ...
    def get_query_features(self, queries, feature_extractor):
        # Get query features
        query_features = []
        for query in queries.values():
            query_input_ids = query['input_ids'].to(feature_extractor.device)
            query_tokens_type_ids = query['tokens_type_ids'].to(feature_extractor.device)
            query_feature = feature_extractor(query_input_ids, token_type_ids=query_tokens_type_ids)[1]
            query_features.append(query_feature.squeeze(0))
        return torch.stack(query_features).detach().cpu()  # Q,H
