...
def main_loop(model, dataloader, epochs=1, batch_iters=1,
    ...
    for epoch in range(epochs):
        ...
        loss_output = f'loss_epoch_{epoch}_{now}.log'
        acc_output = f'acc_epoch_{epoch}_{now}.log'
        with open(loss_output, 'w+') as lo, open(acc_output, 'w+') as ao:
            ...
            for i, batch in pbar:
                ...
                if calc_loss:
                    ...
                    lo.write(model.print_loss() + '\n')
                if calc_acc:
                    description += ' ' + ' '.join([f'{k}:{v}' for k, v in acc.items()])
                    ao.write(' '.join([f'{k}:{v}' for k, v in acc.items()]) + '\n')
        ...
        if save_results:
            ...
            save(all_results, os.path.join('prediction', model_name + '_' + current_time().replace('/', '-') + '_val_acc-' + ' '.join([f'{k}:{v}' for k, v in acc.items()]) + '_val_loss-' + model.print_loss()))
