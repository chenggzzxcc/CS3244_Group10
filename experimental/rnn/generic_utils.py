...
def read(path, cast=str):
    with open(path) as f:
        return [cast(line.strip()) for line in f.readlines()]


def read_pairs(path, separator='\t', cast=(str, str), offset=0, encoding='utf8'):
...
def load_module(path, keep_dataparallel=False, map_location='cpu'):
    ret = torch.load(path, map_location=map_location)
    if not keep_dataparallel and isinstance(ret, DataParallel):
        return ret.module
    return ret
